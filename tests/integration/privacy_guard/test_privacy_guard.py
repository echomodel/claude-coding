"""Integration tests for the privacy-guard agent.
...
class TestLocationDetection:
    ...
    def test_pii_in_gitignored_file(self, person_md, gitignored_pii_repo):
        """PII in gitignored file reported as warning, not error."""
        result = run_privacy_guard(gitignored_pii_repo, person_md_path=person_md)

        assert result["status"] in ("completed", "partial")
        gitignored_findings = [f for f in result["findings"]
                               if f.get("location_type") == "gitignored_file"
                               or f.get("severity") == "warning"
                               or "gitignore" in f.get("note", "").lower()
                               or "secrets/" in f.get("location", "")]
        # Should find something and it should be warning-level
        assert len(gitignored_findings) > 0
...
class TestAuthorIdentity:
    """Agent detects mismatched or problematic git author info."""

    def test_author_email_domain_mismatch(self, person_md, author_mismatch_repo):
        """Flags commits where author email domain differs from configured."""
        result = run_privacy_guard(author_mismatch_repo, person_md_path=person_md)

        assert result["status"] == "completed"
        # Check author_check or findings for the mismatch
        mismatched = result.get("author_check", {}).get("mismatched_authors", [])
        mismatch_findings = [f for f in result["findings"]
                             if f.get("category") == "author_mismatch"
                             or "mismatch" in f.get("note", "").lower()
                             or "megacorp" in f.get("matched_value", "").lower()]
        assert len(mismatched) > 0 or len(mismatch_findings) > 0
