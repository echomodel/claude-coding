...
data, invokes the agent via `claude --agent privacy-guard`, and parses
...
@pytest.fixture
def author_mismatch_repo(test_root):
    """Repo where commit author email domain differs from configured."""
    repo = test_root / "repos" / "mismatch-repo"
    repo.mkdir(parents=True)
    _init_git_repo(repo, author_name="Test Bot", author_email=_e("bot", "test.example"))
    # First commit with configured author
    _add_and_commit(repo, {"README.md": "# Project\n"}, "init")
    # Second commit with mismatched author email domain
    _add_and_commit(repo, {"src/app.py": "pass\n"}, "add app",
                    author_name="Test Bot", author_email=_e("bot", "megacorp.example"))
    return repo


@pytest.fixture
def gitignored_pii_repo(test_root):
    """Repo with PII in a gitignored file."""
    repo = test_root / "repos" / "gitignored-repo"
    repo.mkdir(parents=True)
    _init_git_repo(repo)
    _add_and_commit(repo, {
        ".gitignore": "secrets/\n",
    ...
    # Write PII to gitignored path (don't commit)
    secrets_dir = repo / "secrets"
    secrets_dir.mkdir()
    (secrets_dir / "contacts.txt").write_text("Call Zanzibar Quuxington at 555-123-4567\n")
    return repo
